...
class GtpConnectionGo3(gtp_connection.GtpConnection):
	...
	def try_capture(self, capPoint):
		...
		for point in opponentBlocks:
			...
			if liberty is not None and self.board.check_legal(liberty, self.board.current_player):
	...
	def modified_flood_fill(self, capPoint, colour):
		...
		"""
		fboard = np.array(self.board.board, copy=True)
		pointstack = [capPoint]
		fboard[capPoint] = FLOODFILL
		opponentBlocks = []
		while pointstack:
			current_point = pointstack.pop()
			neighbors = self.board._neighbors(current_point)
			for n in neighbors :
